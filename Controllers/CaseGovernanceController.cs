...
public sealed class CaseGovernanceController(ApplicationDbContext db, ICaseGovernanceService governance) : Controller
{
    ...
    [HttpPost, ValidateAntiForgeryToken, RequireSessionRole("Admin")]
    public async Task<IActionResult> DecideWaiver(int id, bool approve, string reason, CancellationToken ct) { var waiver = await db.CaseDocumentWaivers.FindAsync([id], ct); if (waiver is null) return NotFound(); try { await governance.DecideWaiverAsync(HttpContext.Session.GetInt32("UserId")!.Value, id, approve, reason, ct); TempData["Success"] = "Waiver decision recorded."; } catch (Exception ex) { TempData["Error"] = ex.Message; } return RedirectToAction(nameof(Readiness), new { caseId = waiver.CaseId }); }
    [HttpPost, ValidateAntiForgeryToken, RequireSessionRole("Lawyer")]
