...
public sealed class OperationalController(ApplicationDbContext db, IOperationalUseCaseService service) : Controller
{
    ...
    public async Task<IActionResult> Whereabouts(CancellationToken ct){await service.EscalateOverdueAsync(ct);var items=await db.AttorneyWhereabouts.Include(x=>x.Attorney).Include(x=>x.CalendarEvent).ThenInclude(x=>x!.Case).OrderByDescending(x=>x.CheckedInAtUtc).ToListAsync(ct);var urgent=items.Where(x=>x.CheckedOutAtUtc==null&&x.Status==WhereaboutStatus.DirectorEscalated).SelectMany(x=>db.CalendarEvents.Where(e=>e.CaseId!=null&&e.Case!.LawyerId==x.AttorneyId&&(e.Type==EventType.CourtAppearance||e.Type==EventType.Hearing)&&e.StartDateTime>=DateTime.UtcNow&&e.StartDateTime<=DateTime.UtcNow.AddHours(24))).ToList();ViewBag.UrgentEvents=urgent;var replacementCandidates=new Dictionary<int,IReadOnlyList<ApplicationUser>>();foreach(var e in urgent){replacementCandidates[e.Id]=await service.UrgentReplacementCandidatesAsync(e.Id,ct);}ViewBag.ReplacementCandidates=replacementCandidates;
        var knownVenues=await db.KnownVenues.ToListAsync(ct);
        var pins=items.Where(x=>x.CheckedOutAtUtc==null).Select(x=>{
            double? lat=x.Latitude, lng=x.Longitude;
            if(!lat.HasValue||!lng.HasValue){var kv=knownVenues.FirstOrDefault(v=>x.Venue.Contains(v.Name)||v.Name.Contains(x.Venue));if(kv!=null){lat=kv.Latitude;lng=kv.Longitude;}}
            return new{id=x.Id,lat,lng,name=x.Attorney.FullName,venue=x.Venue,status=x.Status.ToString(),checkedIn=x.CheckedInAtUtc.ToLocalTime().ToString("g")};
        }).Where(x=>x.lat.HasValue&&x.lng.HasValue).ToList();
        ViewBag.MapPinsJson=System.Text.Json.JsonSerializer.Serialize(pins);
        return View(items);}
